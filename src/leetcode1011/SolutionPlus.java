...
class SolutionPlus {
    public int shipWithinDays(int[] weights, int D) {
        ...
        int[] preSum = getPreSum(weights);
        int countSum = preSum[weights.length - 1];
        if (D == 1) {
            return countSum;
        }
        ...
        while (midden > left) {
           int needDay = getNeedDay(preSum,midden);
           if(needDay>D){
               left =midden;
               midden = (left + right) / 2;
           }
    ...
    private int getNeedDay(int[] preSum, int midden) {
        int needDay = 0;
    ...
    private int[] getPreSum(int[] weights) {
